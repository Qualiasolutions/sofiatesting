...
import { getZyprusLocations, getZyprusTaxonomyTerms } from '../lib/zyprus/client';
...
async function testZyprusAPI() {
  ...
  try {
    ...
    if (locations[0]) {
      console.log(`   Sample: ${locations[0].attributes.title} (${locations[0].id})`);
    }
    ...
    if (propertyTypes[0]) {
      console.log(`   Sample: ${propertyTypes[0].attributes.name} (${propertyTypes[0].id})`);
    }
    ...
    console.log(`✅ SUCCESS: Fetched ${outdoorFeatures.length} outdoor features`);
    ...
    console.log('═══════════════════════════════════════');
    console.log(`\nTotal API calls: 6`);
    console.log(`Total resources fetched: ${
      locations.length +
