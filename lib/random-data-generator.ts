...
const cyprusLocations = [
  "Paphos", "Limassol", "Nicosia", "Larnaca", "Ayia Napa", "Paralimni",
  "Tala", "Coral Bay", "Peyia", "Kato Paphos", "Universal", "Emba", "Chloraka",
  "Geroskipou", "Yeroskipou", "Kissonerga", "Latchi", "Polis", "Argaka"
];

const cyprusPropertyTypes = [
  "Villa", "Apartment", "Penthouse", "Townhouse", "Bungalow", "Detached House",
  "Semi-detached House", "Studio", "Maisonette", "Luxury Villa", "Beachfront Villa"
];

const commonCypriotNames = [
  "Andreas & Elena", "George & Maria", "Nicos & Anna", "Costas & Sophia",
  "Michalis & Eleni", "Panayiotis & Despina", "Christos & Georgia", "Marios & Christina",
  "Stavros & Katerina", "Dimitris & Paraskevi", "Savvas & Androula", "Yiannis & Maro"
];

const cypriotSurnames = [
  "Papadopoulos", "Christoforou", "Andreiou", "Georgiou", "Constantinou",
  "Nicolau", "Michaelides", "Charalambous", "Ioannou", "Petrides",
  "Antoniou", "Stephanou", "Pavlou", "Kyriacou", "Neophytou"
];

const propertyFeatures = [
  "Private pool", "Sea views", "Mountain views", "Garden", "Parking space",
  "Air conditioning", "Central heating", "Fireplace", "Storage room", "Guest toilet",
  "Ensuite bathroom", "Walk-in wardrobe", "Balcony", "Terrace", "BBQ area",
  "Solar water heating", "Double glazing", "Alarm system", "Satellite TV", "High-speed internet"
];

export function generateRandomSellerData() {
  const sellerName = commonCypriotNames[Math.floor(Math.random() * commonCypriotNames.length)];
  const surname = cypriotSurnames[Math.floor(Math.random() * cypriotSurnames.length)];
  const registrationNumber = `0/${Math.floor(Math.random() * 9999) + 1}`;
  const location = cyprusLocations[Math.floor(Math.random() * cyprusLocations.length)];
  const propertyType = cyprusPropertyTypes[Math.floor(Math.random() * cyprusPropertyTypes.length)];

  // Generate random dates within next 30 days
  const viewingDate = new Date();
  viewingDate.setDate(viewingDate.getDate() + Math.floor(Math.random() * 30) + 1);
  ...
  const propertyId = Math.floor(Math.random() * 99999) + 10000;
...
export function generateRandomViewingData() {
  ...
  const clientNames = commonCypriotNames.filter(name => name !== baseData.sellerName.split(' ')[0]);
  const clientName = clientNames[Math.floor(Math.random() * clientNames.length)];
  const clientSurname = cypriotSurnames[Math.floor(Math.random() * cypriotSurnames.length)];
  const clientPhone = `99${Math.floor(Math.random() * 900000) + 100000}`;
...
export function getRandomTemplatePrompt(templateType: "registration" | "marketing" | "viewing" = "registration") {
  let data;
  let templateName;
  ...
  if ("bedrooms" in data) {
    const marketingData = data as ReturnType<typeof generateRandomMarketingData>;
    additionalInfo += `
