...
test.describe('Sofia AI Features', () => {
  ...
  test('Document generation works', async ({ page }) => {
    ...
    await page.fill(inputSelector, 'Write me a short essay about Cyprus real estate market opportunities in 2024');
    ...
    const artifactExists = await page.locator('[data-testid="artifact"]').count();
  ...
  test('Property listing with taxonomy works', async ({ page }) => {
    ...
    await page.fill('[data-testid="multimodal-input"]', 'Show me available locations for property listings in Cyprus');
    await page.keyboard.press('Enter');
    ...
    // Create a listing
    await page.fill('[data-testid="multimodal-input"]', 'Create a property listing: 3 bedroom villa in Limassol, 450000 euros, 200 sqm, 2 bathrooms, with swimming pool and garden');
    await page.keyboard.press('Enter');
  ...
  test('Telegram webhook is accessible', async ({ page }) => {
    const response = await page.request.get('http://localhost:3000/api/telegram/webhook');
  ...
  test('WhatsApp webhook responds to verification', async ({ page }) => {
    ...
    const response = await page.request.get('http://localhost:3000/api/whatsapp/webhook', {
      params: {
        'hub.mode': 'subscribe',
  ...
  test('Calculator tools work', async ({ page }) => {
    ...
    await page.fill('[data-testid="multimodal-input"]', 'Calculate transfer fees for a property worth 250,000 euros in Cyprus');
    await page.keyboard.press('Enter');

    await page.waitForTimeout(5000);

    const messages = await page.locator('[data-testid="message"]').allTextContents();
    const hasCalculation = messages.some(msg =>
