...
test.describe("SOFIA Enhanced Formatting Tests", () => {
  test.use({ baseURL: "https://sofiatesting-69ohbse6x-qualiasolutionscy.vercel.app" });
  ...
  test("SOFIA should render numbered lists with proper formatting", async ({ page }) => {
    ...
    const sendButton = page.locator('button[type="submit"]:has([data-testid="arrow-up-icon"])');
    ...
    const response = page.locator('[data-role="assistant"] .sophia-response').last();
  ...
  test("SOFIA should render headings with proper hierarchy", async ({ page }) => {
  ...
  test("SOFIA response container should have proper CSS classes", async ({ page }) => {
    ...
    const responseContainer = page.locator('[data-role="assistant"] .sophia-response').last();
  ...
  test("SOFIA should handle line breaks and paragraphs correctly", async ({ page }) => {
