...
async function testPropertyListingFlow() {
  ...
  const userRequest = "Create a 2 bedroom apartment in Nicosia, Engomi for €250,000";
  ...
  try {
    ...
    console.log(`  - Property Types: ${taxonomyResult.data.propertyTypes.length}`);
    console.log(`  - Indoor Features: ${taxonomyResult.data.indoorFeatures.length}`);
    console.log(`  - Outdoor Features: ${taxonomyResult.data.outdoorFeatures.length}`);
    ...
    const nicosiaLocation = locations.find(loc =>
    ...
    if (!nicosiaLocation) {
      ...
      console.log(`✅ Found location: ${nicosiaLocation.name} (${nicosiaLocation.id})`);
    }

    // Find apartment property type
    const propertyTypes = taxonomyResult.data.propertyTypes;
    const apartmentType = propertyTypes.find(type =>
    ...
    if (!apartmentType) {
      ...
      console.log(`✅ Found property type: ${apartmentType.name} (${apartmentType.id})`);
    }
